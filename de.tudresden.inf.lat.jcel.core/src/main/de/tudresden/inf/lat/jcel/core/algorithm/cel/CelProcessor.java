 ...
 * <li>CR2 : <b>if</b> A &isin; S(X) <b>and</b> A &#8849; &exist; r . B &isin; O
 * <b>and</b> (X,B) &notin; R(r) <b>then</b> R(r) := R(r) &cup;{(X,B)}</li>
 * <li>CR3 : <b>if</b> (X,Y) &isin; R(r) <b>and</b> A &isin; S(Y) <b>and</b>
 * &exist; r . A &#8849; B &isin; O <b>and</b> B &notin; S(X) <b>then</b> S(X)
 * := S(X) &cup; {B}</li>
...
public class CelProcessor implements Processor {
	...
	private static final Logger logger = Logger
			.getLogger(CelProcessor.class.getName());
	...
	public CelProcessor(Set<IntegerAxiom> axioms) {
		if (axioms == null) {
	...
	protected void preProcess(Set<IntegerAxiom> axioms) {
		...
		logger.finer("auxiliary properties created : "
				+ (getIdGenerator().getNextPropertyId() - getIdGenerator()
						.getFirstPropertyId()));
		...
		if (sSofA.containsAll(vecB) && !sSofA.contains(cB)) {
			...
			if (cB.equals(bottom)) {
				...
				for (Integer r : propertySet) {
					...
					Set<Integer> classSet = this.relationSet.getBySecond(r, cA);
	...
	private void processNewEdge(Integer cA, Integer r, Integer cB) {
		for (Integer s : this.transitiveSubsumed.get(r)) {
			...
			for (RI3Axiom axiom : getExtendedOntology()
				...
				Set<Integer> classSet = this.relationSet.getBySecond(t, cA);
			...
			for (RI3Axiom axiom : getExtendedOntology()
				...
				Set<Integer> classSet = this.relationSet.getByFirst(t, cB);
	...
	private void removeAuxiliaryObjectProperties() {
		...
		for (Integer elem : getObjectPropertyGraph().getElements()) {
			if (elem < getIdGenerator().getFirstPropertyId()) {
