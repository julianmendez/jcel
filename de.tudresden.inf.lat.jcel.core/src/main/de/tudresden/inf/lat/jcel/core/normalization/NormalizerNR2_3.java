...
class NormalizerNR2_3 implements NormalizationRule {
	...
	public static Set<IntegerAxiom> apply(IntegerSubClassAxiom classAxiom,
		...
		if (subClass instanceof IntegerObjectSomeRestriction) {
			...
			if (!filler.isLiteral()) {
				...
				IntegerObjectSomeRestriction newExistential = new IntegerObjectSomeRestriction(
						property, newClass);
				ret.add(new IntegerSubClassAxiom(filler, newClass));
				ret.add(new IntegerSubClassAxiom(newExistential, superClass));
			}
		}
		return ret;
	}
