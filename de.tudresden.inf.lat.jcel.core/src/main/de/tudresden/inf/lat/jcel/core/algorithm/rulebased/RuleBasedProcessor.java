...
public class RuleBasedProcessor implements Processor {
	...
	private static final long loggingCount = 0x100000;
	...
	private Deque<REntry> queueR = new ArrayDeque<REntry>();
	private Deque<SEntry> queueS = new ArrayDeque<SEntry>();
	...
	private void applyChanges(List<XEntry> changes) {
		for (XEntry entry : changes) {
			if (entry.isREntry()) {
				REntry rEntry = (REntry) entry;
				boolean changed = this.status.addToR(rEntry);
				if (changed) {
					this.queueR.push(rEntry);
				}
			} else if (entry.isSEntry()) {
				SEntry sEntry = (SEntry) entry;
				boolean changed = this.status.addToS(sEntry);
				if (changed) {
					this.queueS.push(sEntry);
				}
			}
		}
	}
	...
	private boolean isConnectedTo(Integer c, Integer d) {
		...
		while (!toVisit.isEmpty()) {
			...
			for (Integer r : this.status.getObjectPropertiesUsedByClass(elem)) {
	...
	private void prepareQueue() {
		...
		for (Integer className : classNameSet) {
			this.queueS.add(new SEntryImpl(className, className));
			this.queueS.add(new SEntryImpl(className, classTopElement));
		}
	}
	...
	protected void preProcess(Set<ComplexIntegerAxiom> originalAxiomSet) {
		...
		logger.fine("classes read : "
				+ (preprocessor.getIdGenerator().getFirstClassId()));
		logger.fine("object properties read : "
	...
	public boolean process() {
		if (!this.isReady) {
			if (this.queueS.isEmpty() && this.queueR.isEmpty()) {
				postProcess();
				logger.fine(showUsedRules());
				this.isReady = true;
			} else {
				if (this.queueS.size() > this.queueR.size()) {
					SEntry entry = this.queueS.pop();
					List<XEntry> changes = this.chainS
							.apply(this.status, entry);
					applyChanges(changes);
				} else {
					REntry entry = this.queueR.pop();
					List<XEntry> changes = this.chainR
							.apply(this.status, entry);
					applyChanges(changes);
				}
				this.count++;
				if (this.count > loggingCount) {
					this.count = 0;
					logger.fine("[" + " sizes of queueS=" + this.queueS.size()
							+ " queueR=" + this.queueR.size() + " allS="
							+ this.status.getDeepSizeOfS() + " allR="
							+ this.status.getDeepSizeOfR() + " V="
							+ this.status.getSizeOfV() + " subV="
							+ this.status.getDeepSizeOfV() + " ]");
				}
			}
		}
		return !this.isReady;
