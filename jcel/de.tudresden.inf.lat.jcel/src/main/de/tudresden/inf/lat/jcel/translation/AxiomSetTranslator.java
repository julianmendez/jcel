...
public class AxiomSetTranslator {
	...
	protected Map<OWLIndividual, Integer> createIndividualMap(Integer offset,
			TranslationRepository repository) {
		Map<OWLIndividual, Integer> ret = new HashMap<OWLIndividual, Integer>();
		Iterator<OWLIndividual> it = repository.getOWLIndividualList()
				.iterator();
		for (int index = offset; it.hasNext(); index++) {
			OWLIndividual current = it.next();
			ret.put(current, index);
		}
		return ret;
	}
	...
	public Map<OWLIndividual, Integer> getIndividualMap() {
	...
	public Set<IntegerAxiom> translate(Set<OWLAxiom> axiomSet)
		...
		for (OWLAxiom axiom : axiomSet) {
			...
			if (translation != null) {
				...
				logger.fine("Omitting axiom '" + axiom + "'.");
			}
		}
		return ret;
	}
