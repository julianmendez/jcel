 ...
 * <p>
 * <ul>
 * <li>CR3 : <b>if</b> &exist; r <i>.</i> A &#8849; B &isin; T, (r,x,y) &isin;
 * R, <u>(y,A) &isin; S</u>, (x,B) &notin; S <br />
 ...
 */
public class CR3SBasicRule implements SObserverRule {

	public CR3SBasicRule() {
	}
	...
	private List<XEntry> applyRule(ClassifierStatus status, Integer y, Integer a) {
		...
		for (Integer r : status.getObjectPropertiesUsedByClass(y)) {
			for (GCI3Axiom axiom : status.getExtendedOntology()
					.getGCI3rAAxioms(r, a)) {
				for (Integer x : status.getRelationBySecond(
						axiom.getPropertyInSubClass(), y)) {
